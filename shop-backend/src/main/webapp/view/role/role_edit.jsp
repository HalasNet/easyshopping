...
<div class="content_c">
  <div class="content_c_div">
   ...
   <form id="roleForm" action="role!update" method="post" onsubmit="return checkSave();">
      <table width="420" border="0" align="center" cellpadding="0" cellspacing="0">
	      <tr>
	        <td width="30%" align="right">角色名称：</td>
	        <td width="70%"><input  class="box_170" type="text" name="model.roleName" id="roleName" maxlength="20"  value="${role.roleName}"><input type="hidden" name="model.id" id="id"  value="${role.id}">
				  ...
				  <tr>
				    <td width="200">
				      <script type="text/javascript">
				        d = new dTree('d');
						d.add(0,-1,'全部权限：');
						<s:iterator value="authTreeList">
						<s:if test='%{authority.parent==null }'>
							d.add(<s:property value="authority.id" />,0,'authority','<s:property value="authority.id" />','<s:property value="authority.name" />','<s:property value="isChecked" />');
						</s:if>
						<s:else>
							d.add(<s:property value="authority.id" />,<s:property value="authority.parent.id" />,'authority','<s:property value="authority.id" />','<s:property value="authority.name" />','<s:property value="isChecked" />');     
						</s:else>
      ...
      <tr>
        ...
        <td width="70%"><textarea name="model.remark" id="remark" rows="6" class="box_h">${role.remark}</textarea></td>
      </tr>
      <tr>
