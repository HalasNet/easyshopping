...
function Node(id, pid, name, permission, url, title, target, icon, iconOpen, open , cname, cvalue, cshow, cchecked,cdisabled) {
...
dTree.prototype.add = function(id, pid, name, permission,url, title, target, icon, iconOpen, open, cname, cvalue, cshow, cchecked,cdisabled) {

	this.aNodes[this.aNodes.length] = new Node(id, pid, name, permission,url, title, target, icon, iconOpen, open, cname, cvalue, cshow, cchecked, cdisabled);
...
dTree.prototype.node = function(node, nodeId) {
	...
	if (this.config.useIcons) 
	{
    	...
    	{ 
        /**组装checkbox开始*/ 
        checkboxSyntax = "<input type='checkbox' desc='" + node.cshow + "' name='rightName'" + "' id='" + node.cname + 

        "_" + node.id + "' value='" + node.permission + "' onClick='javascript: " + this.obj + ".checkNode(" + 

        node.id+","+node.pid+","+node._hc + ",this.checked);' "; 
        //是否被选中 
        ...
        str += '<img id="i' + this.obj + nodeId + '" src="' + ((node._io) ? node.iconOpen : node.icon) + '" alt="" style="display:none"/>';
...
dTree.prototype.checkNode = function(id,pid,_hc,checked) { 
    ...
    if(!this.isHaveBNode(id,pid)){ 
        ...
        if(checked){ 
            ...
            // 去掉选中将其父节点和根节点(当父节点没有同级选中时)去掉选中 
...
dTree.prototype.checkPNode = function(pid,ischecked) {  
    ...
    for (var n=0; n<this.aNodes.length; n++) { 
        ...
        if (this.aNodes[n].pid!=-1&&this.aNodes[n].id == pid) {         
            ...
            //为了增加取消最低层的选择框时,把最上层的根节点也取消,当其它同级目录没被选中时.开始,09.01.20 李冬
